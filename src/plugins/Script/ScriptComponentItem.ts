...
export const ScriptComponentItem = (editor, commonTraits) => {
    ...
    editor.DomComponents.addType('customScriptItem', {
        isComponent: (el) => el.hasAttribute && el.hasAttribute('data-scriptable'),
        model: {
            defaults: {
                attributes: { jscript: `let res = 1 + 3; console.log('result is', res);` },
                traits: [
                    {
                        // type: 'text',
    ...
    editor.TraitManager.addType('jcodemirror-editor', {
        ...
        onEvent({ elInput, component, event }) {
            let code_to_run;
            console.log('elInput');
            if (codemirrorEnabled) code_to_run = elInput.querySelector('.CodeMirror').CodeMirror.getValue();
    ...
    editor.BlockManager.add('scriptItem', {
        category: 'Scriptable',
        label: 'Scriptable Item',
