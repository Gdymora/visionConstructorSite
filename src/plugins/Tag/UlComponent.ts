export const UlComponent = (editor, commonTraits) => {
    editor.DomComponents.addType('ul-tag', {
        isComponent: el => {
            if (el.tagName === 'UL') {
                return { type: 'ul-tag' };
            }
        ...
        model: {
            defaults: {
                tagName: 'ul',
                traits: [
                    { type: 'text', label: '<ul>', name: 'content', changeProp: true },
                ],
    ...
    editor.BlockManager.add('ul-tag', {
        label: 'Ul Component',
        category: 'Tage',
        content: {
            type: 'ul-tag',
